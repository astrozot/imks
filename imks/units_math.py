...
def ceil(x):
    if isinstance(x, Value):
        return Value(math.ceil(x.value), x.unit)
    else:
        return math.ceil(x)
...
def fabs(x):
    if isinstance(x, Value):
        return Value(math.fabs(x.value), x.unit)
    else:
        return math.fabs(x)
...
def floor(x):
    if isinstance(x, Value):
        return Value(math.floor(x.value), x.unit)
    else:
        return math.floor(x)
...
def fmod(x, y):
    if isinstance(x, Value) or isinstance(y, Value):
        x1 = Value(x)
        y1 = Value(y)
        return Value(math.fmod(x1.value, y1.value), x1.unit - y1.unit)
    else:
...
def round(x):
    if isinstance(x, Value):
        return Value(_round(x.value), x.unit)
    else:
        return _round(x)
...
def sqrt(x):
    if isinstance(x, Value):
        return Value(math.sqrt(x.value), x.unit / 2)
    else:
        return math.sqrt(x)
